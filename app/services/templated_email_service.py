...
class TemplatedEmailService:
    ...
    def send_otp_email(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Send OTP verification email using SES template"""
        try:
            # Extract data directly from the request (matching API documentation)
            to_email = data.get('to')
            username = data.get('username', 'User')
            otp_code = str(data.get('otp_code', '123456'))
            expiry_minutes = data.get('expiry_minutes', 10)
            
            if not to_email:
                return {'success': False, 'error': 'Missing required field: to'}
            
            if not otp_code or otp_code == '':
                return {'success': False, 'error': 'Missing required field: otp_code'}
            
            # Template variables for SES template
            template_variables = {
                'user_name': username,
                'otp_code': otp_code,
                'expiry_minutes': str(expiry_minutes),
                'support_email': self.default_sender,
                'brand_logo': self.default_brand_logo,
            ...
            return {
                'success': True,
                'message': f'OTP email sent successfully',
                'template_type': 'otp',
                'to': to_email,
                'otp_code': otp_code,
            ...
            if e.response['Error']['Code'] == 'TemplateDoesNotExist':
                return {'success': False, 'error': 'OTP email template not found. Please create templates first.'}
            logger.error(f"OTP email SES error: {e}")
            return {'success': False, 'error': f'SES error: {str(e)}'}
        except Exception as e:
            logger.error(f"OTP email error: {e}")
            return {'success': False, 'error': str(e)}
    
    def send_welcome_pack_email(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not company_name:
                return {'success': False, 'error': 'Missing required field: company_name'}
                
            if not course_name:
                return {'success': False, 'error': 'Missing required field: course_name'}
            ...
            if e.response['Error']['Code'] == 'TemplateDoesNotExist':
                return {'success': False, 'error': 'Welcome email template not found. Please create templates first.'}
            logger.error(f"Welcome pack email SES error: {e}")
            return {'success': False, 'error': f'SES error: {str(e)}'}
        except Exception as e:
            logger.error(f"Welcome pack email error: {e}")
            return {'success': False, 'error': str(e)}
    
    def send_order_confirmation_email(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Send order confirmation email using SES template"""
        try:
            # Extract data directly from the request (matching API documentation)
            to_email = data.get('to')
            username = data.get('username', 'Customer')
            order_id = data.get('order_id', 'ORD-12345')
            order_total = data.get('order_total', '$0.00')
            ...
            if not order_id:
                return {'success': False, 'error': 'Missing required field: order_id'}
                
            if not order_total:
                return {'success': False, 'error': 'Missing required field: order_total'}
            
            # Calculate items summary for template
            items_summary = ""
            if items:
                for item in items:
                    ...
                    items_summary += f"• {name} (x{quantity}) - {price}\n"
            else:
                items_summary = f"• Order {order_id} - {order_total}"
            
            # Template variables for SES template
            template_variables = {
                'customer_name': username,
                'order_number': order_id,
                'order_date': '2025-10-04',  # You can make this dynamic
            ...
            if e.response['Error']['Code'] == 'TemplateDoesNotExist':
                return {'success': False, 'error': 'Order confirmation template not found. Please create templates first.'}
            logger.error(f"Order confirmation email SES error: {e}")
            return {'success': False, 'error': f'SES error: {str(e)}'}
        except Exception as e:
